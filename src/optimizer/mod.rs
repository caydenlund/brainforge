...
pub fn optimize(
    ...
    let optimizers = {
        let mut optimizers: Vec<
            fn(Vec<IntermediateInstruction>) -> (Vec<IntermediateInstruction>, bool),
        > = vec![];
        if opts.coalesce {
            optimizers.push(coalesce);
        }
        if opts.simple_loops {
            optimizers.push(make_simple_loops);
        }
        if opts.scans {
            optimizers.push(make_scans);
        }
        optimizers
    };
