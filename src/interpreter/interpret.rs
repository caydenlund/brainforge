...
pub fn interpret(src: &[Instruction], mem_size: usize) {
    let mut state = RuntimeState::new(mem_size);

    while state.instr < src.len() {
        match src[state.instr] {
            Instruction::Left => state.ptr = (state.ptr + 1) % mem_size,
            Instruction::Right => state.ptr = (state.ptr + mem_size - 1) % mem_size,
